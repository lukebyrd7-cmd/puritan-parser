...
| v1.3.6a Hebrew weak verbs | Complete: 37 Gesenius-backed recognition charts across eleven weak-class navigation IDs, with class/stem/form filters and explicit limited examples | `reference-data.js`, Reference renderer, Settings sources, recognition compatibility, tests, docs | Gesenius-Kautzsch-Cowley 1910, §§62–78 pp. 164–219 and Paradigms D–P pp. 514–529, page-image verified | Source/ID/class/root/comparison/NFC/pointing/RTL/filter/About/recognition/isolation tests; desktop/mobile visual QA | Starred/bracketed alternatives and doubly weak complexity | Additional rare variants and exhaustive doubly weak coverage |
| v1.3.6b Hebrew suffixes and nominal morphology | Implemented for review: 16 Gesenius-backed charts covering Construct State, pronominal suffixes on nouns and prepositions, limited object suffixes on verbs, Segolate Nouns, reducible-vowel patterns, and selected Irregular Nouns | `reference-data.js`, Reference renderer, Settings sources, recognition isolation, tests, docs | Gesenius-Kautzsch-Cowley 1910 supplies row forms; Pratico–Van Pelt 2019 guides visible terminology and organization; §§58–59, 89, 91, 93, 95–96, 103, pp. 155–160, 247, 254–285, 300–305, page-image verified | Source/ID/form-hash/metadata/PGN/NFC/pointing/RTL/filter/typography/About/recognition/isolation tests; desktop/mobile visual QA | Wide comparison tables and multiple lexical noun classes | Automatic drills, exhaustive suffix systems, and the full irregular-noun inventory |
| v1.3.7 Grammar Handbook rebuild | Implemented for review: 12 Greek and 13 Hebrew reading-oriented articles in six stable sections per language, including fuller sequential-form explanation | `handbook-data.js`, `reference-data.js`, Reference renderer, Settings sources, tests, docs | Merkle/coauthors and Pratico–Van Pelt organize; Machen and Gesenius support foundational content; app workflows are explicit syntheses | Registry/source/search/route/link/Unicode/a11y/hash tests; desktop/mobile browser QA | Duplicating chart explanations avoided through stable-ID links | Advanced/specialized debates, exhaustive taxonomies, additional paradigms |
...
The normalized v1.3.3/v1.3.4 Greek registries and v1.3.5/v1.3.6a/v1.3.6b Hebrew registries have exact row-level citations for their stated coverage. The v1.3.7 Handbook now carries article-level source-role and location metadata, while app-authored workflows are identified as synthesis. Legacy explanatory prose and older snapshots outside the focused Handbook and normalized chart registries still lack equivalent verification; the audit continues to distinguish them from source-backed material.

## v1.3.7 implemented Handbook architecture

The visible Handbook no longer renders legacy topic fragments or full paradigm tables. Six section IDs per language filter one normalized registry: Greek uses `greek-nouns-agreement`, `greek-indicative-system`, `greek-voice-mood`, `greek-nonfinite`, `greek-markers`, and `greek-reading`; Hebrew uses `hebrew-nouns-construct`, `hebrew-suffixes-prepositions`, `hebrew-qal-conjugations`, `hebrew-derived-stems`, `hebrew-weak-verbs`, and `hebrew-reading`.

Article navigation renders one active body, supports section filtering and debounced title/alias/keyword search, and uses `/grammar?article=<stable-id>` for direct links. Related articles and existing Paradigm Charts use stable IDs rather than visible labels. Reverse chart links are derived from the same registry, so there is no second mapping to drift. Full source details live under Settings → About & Sources at `grammar-handbook-sources`; ordinary articles contain only “View source notes.”

The rebuild deliberately removed empty Morphology/Parsing shells and folded their useful vocabulary into aliases and article-level recognition cues. It did not remove the top-level compatibility aliases used by Reader, Learn, older links, or Reference Search. It did not add charts, drills, migrations, or stored state.
