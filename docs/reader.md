...
Original and English are one exclusive two-state choice with radio-group semantics; exactly one mode is active. Visibility changes capture a genuinely visible original or English verse through a shared canonical chapter-and-verse identity, restore it synchronously before focus moves, refine the position after layout, and focus controls with `preventScroll`. Request and restoration generations prevent stale location, translation, or scheduled scroll work from overwriting newer choices.
