...
The shared Greek/Hebrew Reader supports two explicit modes: Continuous and One chapter at a time. Continuous is the default when no valid saved mode exists. Existing `chapter` and `continuous` values remain authoritative, so users who selected one-chapter reading retain it. The choice lives under Settings → Reader rather than in the always-visible Reader controls; the restrained Reader options link opens that destination and a Return to Reader action restores the reading flow. Continuous mode renders the selected chapter with nearby chapters from the same book, then incrementally loads another adjacent chapter near a scroll boundary. The rendered window is bounded to five chapters, uses the existing `language/book/chapter` memory cache, preserves canonical order, and rejects duplicates.
