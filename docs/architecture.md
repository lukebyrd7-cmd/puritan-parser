...
Finite maintenance sessions copy a unique ordered entry snapshot and do not rebuild after each answer. Reinforcement order is F, D, C, then stale B/A fallback; Random accepts a deterministic random source for tests; book focus consumes the existing cached Book Progress occurrence vocabulary rather than scanning Scripture. Unlimited sessions extend only after the current bounded pool has been traversed and are not persisted.
