...
The startup controller treats theme, shell, router, the active route bundle, and route-required data as critical. Whole-vocabulary hydration, inactive feature registries, service-worker update checks, and cache warming are deferred or noncritical. The shell announces ordinary loading, explains a delayed load after 4.5 seconds, and shows a recoverable error with Retry and home actions when critical startup fails. Retry advances a startup generation so stale completions cannot reveal or route over the newer attempt; bootstrap, router listeners, application listeners, and service-worker registration are idempotent. Retry does not clear localStorage or Reader location.

Service-worker installation remains all-or-nothing for the shell precache, followed by `skipWaiting`, old-cache cleanup, and `clients.claim`. Navigation is network-first with cached shell fallback; JSON is network-first and cached only after a successful response. Versioned JavaScript and CSS requests require an exact query match, preventing an old controlling worker from returning old bytes for a new HTML version. Stable unversioned assets retain query-insensitive cache lookup. Registration failure is reported to the console as unavailable offline support but does not block the functioning app; `pageshow` after Back/Forward Cache restoration and `online` only recheck registration and never force a reload.
...
The utility is loaded as small core code, but no derived Hebrew index is constructed synchronously during startup. Global Search derives terms through the shared idle/single-flight preparation path. Reader Search derives terms when the Hebrew verse index is first searched lexically and reuses the in-memory result. The 20 MB Reader search JSON is installed in the versioned offline cache but remains lazy in application memory. Derived forms are not persisted in localStorage and are never displayed in the Reader.
...
Hebrew Interlinear is a language-specific display preference stored inside the existing `pp_reader_adaptive_settings` record. Standard is the safe default. Interlinear data is loaded into application memory for Hebrew Original text when the preference is active, or for one requested chapter when a Standard-mode Word Details selection needs its trusted lexical segments. It shares the Reader's in-flight and completed request behavior and participates in the same five-chapter continuous window when displayed. A failed enrichment request leaves Standard Hebrew readable and offers retry. Every generated interlinear chapter is installed in the versioned offline cache, while Reader memory use remains chapter-lazy.
...
Hard-reload place restoration uses one generation-controlled lifecycle: pending, waiting for data/layout, applying, verifying, complete, or cancelled. It begins only after the active Reader route has rendered the requested language, book, chapter, mode, text layer, and canonical verse into a nonzero `.reader-text` viewport. It does not wait indefinitely for a remote font; font completion and measured pane resize are verification signals for the one bounded correction. The verse-relative offset is primary, raw pane/window offsets are backward-compatible fallbacks, invalid offsets resolve to the verse start, and a measured error beyond 8 CSS pixels permits one correction. Wheel, touch, pointer, keyboard scrolling, explicit navigation, route changes, and user display changes cancel pending work so restoration cannot fight user input.
...
The service worker installs the provider, both manifests, and every OEB and WEB chapter in the versioned offline cache. Reader memory use remains chapter-lazy, and Original / English switching never requires a prior visit to that chapter.
...
Practice session startup reads status directly from the already-normalized vocabulary-learning record map. It does not normalize the whole store once per vocabulary entry and does not build the global Search corpus. Session assembly remains bounded by the configured finite size or the existing 100-card lazy batch for continue-until-stopped practice.

Hebrew Reader identity uses two deliberately separate source layers. The generated Reader token's numeric lemma remains the canonical vocabulary and learning key. The interlinear segment sequence, attached during Interlinear display or loaded on demand for the selected Standard-mode chapter, supplies the exact lexical display lemma and lexical part of speech: the last non-suffix lexical-content segment wins, while preceding segments are prefixes and following `Sp` segments are suffixes. Whole-token morphology remains occurrence data and cannot overwrite lexical identity. Greek keeps its existing direct token-to-lemma resolution because it does not use this segmented Hebrew pipeline.
...
The versioned cache in `sw.js` must include `index.html`, styling, icons, the small content manifest, and the exact query-version URL for every browser startup module listed by `src/main.js`, including migrations, router, model helpers, content helpers, and storage modules. It also includes the runtime vocabulary, glosses, Reader corpora, search indexes, Hebrew interlinear chapters, and translations required for a fresh installed-PWA launch with no network. Large data remains lazy in application memory even though it is installed in Cache Storage.
...
Installed core JSON uses a cache-first strategy tied to the service-worker version, so offline initialization does not wait for a failed network request and app/data versions cannot mix. A successful new installation activates only after the full required inventory is present; activation removes obsolete Puritan Parser caches without touching user storage.
