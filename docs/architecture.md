...
- `index.html` owns the page markup and loads scripts in dependency order.
- `app.js` now contains shared constants and application state only.
...
4. Prefer incremental moves over rewrites. The Phase 1 modules intentionally retain the existing function names and browser-global loading pattern to keep review simple and behavior stable.
