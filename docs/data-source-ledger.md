...
The runtime asset contains existing contributor-authored CC0 records and Abbott-Smith-derived public-domain records side by side with record-level source metadata. Abbott-Smith records contain no entry prose. Swanson remains a private verification-only source: the checked-in verification manifest stores stable IDs and result classifications only, never definition wording or paraphrases derived from it.
