...
v1.5 stabilization is implemented for review. It fixes continuous-Reader text-toggle position loss, replaces the bottom mobile controls with a restrained top toolbar, deduplicates and prefetches adjacent chapter requests, phases Progress so core metrics paint before whole-Bible readiness work, defers inactive Reference rendering, and repairs nested-route startup asset resolution. Release validation remains the current focus; no feature expansion is included.
...
Measurements used the local static development server and the Codex in-app browser at 390×844. They include roughly 1.2 seconds of browser-automation overhead and are directional local-development measurements, not production benchmarks. Before stabilization, Progress painted its shell at approximately 3.37 seconds and did not show the complete/core summary until 6.07 seconds. After phasing, its shell painted at approximately 3.12 seconds, core values at 3.24 seconds, and full readiness at 5.26 seconds: useful values appeared about 2.83 seconds sooner. A continuous Reader text toggle measured approximately 2.40 seconds before and 1.97 seconds after under the same automation, while the material correction was removal of the chapter/top jump. Adjacent-load improvement is evidenced primarily by one shared in-flight request per chapter and idle loading of only the immediate chapters outside the five-chapter window; the browser harness did not provide a reliable isolated boundary-render timing.
