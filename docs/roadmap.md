...
v1.8 connects the existing Learn review and on-demand practice foundations into one daily-practice model. Scheduled reviews remain required SRS work; voluntary known-word maintenance is prominent, counts unique words toward the existing per-language goal, and defaults to leaving the schedule unchanged. Deterministic A–F mastery summaries guide reinforcement priority from recorded evidence without replacing vocabulary status or adding a second scheduler. Progress reports the same derived grades and daily union. No linguistic or Reader data is in scope.

v1.7.2 hardens the startup/restoration lifecycle without changing Reader scrolling. Critical route startup is generation-controlled and recoverable, optional hydration and offline-support registration cannot blank the shell, versioned startup assets use exact cache matching, and hard reload restores one canonical verse-relative place with bounded verification and user-input cancellation. The existing location/settings keys and interlinear corpus remain unchanged. Physical iPhone Safari, iPhone Chrome, and installed/PWA startup/restoration validation are still required before publication approval.
