...
test('v1.3.1 app shell and service worker keep Reference assets reachable without stale versioning', () => {
  ...
  assert.match(sw, /puritan-parser-v72-v1\.7-hebrew-interlinear-5/, 'service-worker cache version is bumped for Hebrew interlinear changes');
  assert.match(fs.readFileSync('index.html', 'utf8'), /src\/main\.js\?v=v1\.7-hebrew-interlinear-5/, 'startup query string is bumped with the cache');
