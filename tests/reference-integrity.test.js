...
test('v1.3.1 app shell and service worker keep Reference assets reachable without stale versioning', () => {
  ...
  assert.match(sw, /puritan-parser-v101-v1\.9-unified-glosses-data-trust/, 'service-worker cache version is bumped for Reader, gloss, and practice fixes');
  assert.match(fs.readFileSync('index.html', 'utf8'), /src\/main\.js\?v=v1\.9-unified-glosses-data-trust-5/, 'startup query string is bumped with the cache');
