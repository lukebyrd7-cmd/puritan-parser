...
test('v1.3.1 app shell and service worker keep Reference assets reachable without stale versioning', () => {
  ...
  assert.match(sw, /puritan-parser-v78-v1\.8-daily-practice-mastery/, 'service-worker cache version is bumped for daily practice and mastery');
  assert.match(fs.readFileSync('index.html', 'utf8'), /src\/main\.js\?v=v1\.8-daily-practice-mastery/, 'startup query string is bumped with the cache');
