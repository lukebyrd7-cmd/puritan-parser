...
test('v1.3.1 app shell and service worker keep Reference assets reachable without stale versioning', () => {
  ...
  assert.match(sw, /puritan-parser-v67-v1\.6-hebrew-search-1/, 'service-worker cache version is bumped for Hebrew search changes');
  assert.match(fs.readFileSync('index.html', 'utf8'), /src\/main\.js\?v=v1\.6-hebrew-search-1/, 'startup query string is bumped with the cache');
