...
test('v1.3.1 app shell and service worker keep Reference assets reachable without stale versioning', () => {
  ...
  assert.match(sw, /puritan-parser-v105-v1\.9\.1-hebrew-vocabulary-audit/, 'service-worker cache version is bumped for Hebrew vocabulary corrections');
  assert.match(fs.readFileSync('index.html', 'utf8'), /src\/main\.js\?v=v1\.9\.1-hebrew-vocabulary-audit-1/, 'startup query string is bumped with the cache');
