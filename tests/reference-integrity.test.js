...
test('v1.3.1 app shell and service worker keep Reference assets reachable without stale versioning', () => {
  ...
  assert.match(sw, /puritan-parser-v77-v1\.7\.2-mobile-startup-reader-restoration/, 'service-worker cache version is bumped for startup and Reader restoration');
  assert.match(fs.readFileSync('index.html', 'utf8'), /src\/main\.js\?v=v1\.7\.2-mobile-startup-reader-restoration/, 'startup query string is bumped with the cache');
