...
test('v1.3.1 app shell and service worker keep Reference assets reachable without stale versioning', () => {
  ...
  assert.match(sw, /v115-\$\{APP_VERSION\}/, 'service-worker cache version is bumped for the reliability audit');
  assert.match(fs.readFileSync('index.html', 'utf8'), /src\/main\.js\?v=v1\.9\.7-reliability-performance-trust-1/, 'startup query string is bumped with the cache');
