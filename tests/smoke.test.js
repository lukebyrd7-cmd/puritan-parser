...
test('smoke: nested hard refreshes resolve startup assets from the app root', () => {
  ...
  const sw = fs.readFileSync('sw.js', 'utf8');
  assert.match(html, /href="\/styles\.css\?v=v1\.9\.2-greek-vocabulary-audit-8"/);
  assert.match(html, /src="\/src\/main\.js\?v=v1\.9\.2-greek-vocabulary-audit-8"/);
  ...
  assert.match(main, /PURITAN_PARSER_ASSET_VERSION = 'v1\.9\.2-greek-vocabulary-audit-8'/);
  ...
  assert.match(sw, /'\.\/styles\.css\?v=v1\.9\.2-greek-vocabulary-audit-8'/);
  assert.match(sw, /'\.\/src\/main\.js\?v=v1\.9\.2-greek-vocabulary-audit-8'/);
  assert.match(sw, /ignoreSearch: !isVersionedStartupAsset/);
});
