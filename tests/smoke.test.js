...
test('smoke: versioned startup assets cannot match an older query version', () => {
  const sw = fs.readFileSync('sw.js', 'utf8');
  assert.match(sw, /const APP_VERSION = 'v1\.9\.5-canonical-vocabulary-forms-2'/);
  assert.match(sw, /STARTUP_SCRIPT_PATHS\.map\(path => `\.\/\$\{path\}\?v=\$\{APP_VERSION\}`\)/);
...
test('smoke: nested hard refreshes resolve startup assets from the app root', () => {
  ...
  const sw = fs.readFileSync('sw.js', 'utf8');
  assert.match(html, /href="\/styles\.css\?v=v1\.9\.5-canonical-vocabulary-forms-2"/);
  assert.match(html, /src="\/src\/main\.js\?v=v1\.9\.5-canonical-vocabulary-forms-2"/);
  ...
  assert.match(main, /PURITAN_PARSER_ASSET_VERSION = 'v1\.9\.5-canonical-vocabulary-forms-2'/);
