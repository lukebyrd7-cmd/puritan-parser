...
function renderAboutSources(){
  ...
  shell.innerHTML = `<article class="about-sources-page">
    ...
    <section id="vocabulary-mastery-methodology"><h2>Vocabulary Mastery Methodology</h2><p>Mastery grades summarize recorded recall evidence and are intended to guide practice priority. A Strong or Familiar grade requires established successful recall; sparse or newly known records normally begin as Developing. Recent and repeated misses lower the grade, including maintenance misses made while schedule adjustment is Off. The calculation uses only local scheduled-review and maintenance history, current interval evidence, and deterministic boundaries; it does not use AI, response timing, or manually assigned grades.</p><p>Scheduled reviews and voluntary maintenance practice remain distinct. Daily goal progress is the unique union of words answered in either mode for the local study date. Category counts show unique scheduled and maintenance words separately, so a word may appear in both categories but counts once toward the combined goal. Maintenance events are stored with the existing word record and bounded to the 20 most recent maintenance outcomes. Older records remain valid without migration.</p></section>
