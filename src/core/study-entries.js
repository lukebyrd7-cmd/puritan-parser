...
(function(root, factory){
  ...
  function aggregateLemmaFrequency(entries = [], representative = entries[0]){
    // Form rows may already carry a lemma-level frequency from the source data.
    // Summing those values would multiply the same lemma count by every attested
    // form, so prefer explicit lemma-level fields and otherwise use the best
    // available representative/highest frequency as a conservative aggregate.
    const explicit = entries
      .flatMap(entry => [entry?.lemmaFreq, entry?.lemmaFrequency, entry?.frequencyLemma])
      .map(numericFrequency)
      .filter(Boolean);
    if(explicit.length) return Math.max(...explicit);

    const frequencies = entries.map(entry => numericFrequency(entry?.freq)).filter(Boolean);
    if(!frequencies.length) return numericFrequency(representative?.freq);

    const unique = new Set(frequencies);
    if(unique.size < frequencies.length) return Math.max(...frequencies);

    return Math.max(numericFrequency(representative?.freq), ...frequencies);
  }
  ...
  function createLemmaStudyEntry(entries){
    ...
    const freq = aggregateLemmaFrequency(originals, representative);
    return {
      id: `lemma:${lang}:${lemma}`,
  ...
  return { aggregateLemmaFrequency, groupEntriesByLemma, getStudyEntries, isLemmaStudyEntry, getStudyEntryOriginals, getStudyEntrySearchText };
});
